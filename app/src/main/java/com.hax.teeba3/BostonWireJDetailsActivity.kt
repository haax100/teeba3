...
@AndroidEntryPoint
class BostonWireJDetailsActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContent {
            Teeba3Theme {
                BostonWireJDetailsScreen(intent.getStringExtra("deviceName") ?: "Unknown")
            }
        }
    }
}
...
@Composable
fun BostonWireJDetailsScreen(deviceName: String) {
    val specifications = when (deviceName) {
        "V-18™ & V-14™ ControlWire™ Guidewires" -> listOf(
            "Length: 150cm, 180cm",
            "Coating: Hydrophilic",
            "Size: 0.018 inch"
        )
        "Amplatz Super Stiff™ Guidewire" -> listOf(
            "Length: 150cm, 260cm",
            "Coating: PTFE",
            "Size: 0.035 inch"
        )
    ...
    val imageResId = when (deviceName) {
        "V-18™ & V-14™ ControlWire™ Guidewires" -> R.drawable.boston_v18_v14_image
        "Amplatz Super Stiff™ Guidewire" -> R.drawable.boston_amplatz_image
        else -> null
    }
